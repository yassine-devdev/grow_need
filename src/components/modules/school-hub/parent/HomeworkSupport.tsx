...
const HomeworkSupport: React.FC = () => {
  ...
  const handleGetHelp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await aiProvider.generateContent(question, {
      ...
      setError(`Sorry, an error occurred while trying to get help. Please check your Ollama connection and try again.`);
    } finally {
      setIsLoading(false);
    }
  };
