...
export class VectorDBBridge {
  ...
  async testConnection(): Promise<boolean> {
    try {
      ...
      // Only log connection errors in development mode to reduce console spam
      if (process.env.NODE_ENV === 'development') {
        console.warn('Vector DB connection test failed (this is expected if no backend is running):', error.message);
      }
      return false;
    }
  }
