...
export class EnhancedAIProvider {
  ...
  private lastConnectionAttempt: Date = new Date(0);
  private connectionRetryDelay: number = 30000; // 30 seconds between retries
  ...
  /**
   * Check if vector database is available with retry logic
   */
  private async checkVectorDBAvailability(): Promise<void> {
    const now = new Date();

    // Skip if we recently attempted a connection
    if (now.getTime() - this.lastConnectionAttempt.getTime() < this.connectionRetryDelay) {
      return;
    }
    ...
    try {
      ...
      if (this.isVectorDBAvailable) {
        ...
        this.connectionRetryDelay = 30000;
      }
    } catch (error) {
      // Only log once per retry cycle to reduce console spam
      if (process.env.NODE_ENV === 'development') {
        console.warn('Vector database not available, falling back to standard AI generation');
      }
      this.isVectorDBAvailable = false;
      // Increase retry delay exponentially (max 5 minutes)
      this.connectionRetryDelay = Math.min(this.connectionRetryDelay * 1.5, 300000);
    }
  }
  ...
  async generateEducationalContent(
    ...
    // Note: useRAG parameter is kept for future RAG integration
