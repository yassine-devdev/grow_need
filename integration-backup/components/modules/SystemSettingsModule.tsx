...
const SystemSettingsModule: React.FC = () => {
  ...
  const [activeReportId, setActiveReportId] = useState<string>(settingsData.General[1].id); // Default to Branding
      ...
      <div className="flex items-center gap-2 p-2 shrink-0 system-settings-header-bordered overflow-x-auto">
        {L1_TABS.map(tab => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 shrink-0 ${activeL1 === tab.id ? 'active text-white' : 'text-gray-400'}`}
          >
            {tab.icon}
            {tab.name}
          </button>
